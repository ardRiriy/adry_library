...
use crate::{data_structure::segtree::monoids::{MapMonoid, Monoid}, utils::versatility_functions::unpack_range};
...
impl<T: MapMonoid> LazySegmentTree<T> {
    pub fn new(len: usize) -> Self {
        ...
        Self { n, len, data: vec![T::id(); 2*n], lazy: vec![T::identity_map(); 2*n] }
    }
    ...
    fn get_range_rec(&mut self, l: usize, r: usize, k: usize, seg_l: usize, seg_r: usize) -> <T::M as Monoid>::S {
        self.eval(k);
        if seg_r <= l || r <= seg_l {
            return T::id();
        } 
    ...
    fn update_range_rec(&mut self, l: usize, r: usize, k: usize, f: &T::F, seg_l: usize, seg_r: usize) {
        self.eval(k);
        if l <= seg_l && seg_r <= r {
